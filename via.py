"""Основной Telegram-бот для управления VPN-клиентами через 3x-ui API.

Проект поддерживает:
- создание и обновление VPN-клиентов;
- генерацию QR-кодов и ссылок для подключения;
- систему инвайтов и рефералов;
- базовую админ-панель для рассылок.
"""
...
# Домен и базовый URL можно задать через переменную окружения DOMAIN.
# Если значение не задано, используются безопасные заглушки.
DOMAIN = os.getenv("DOMAIN", "https://your-domain.example")
WEBSITE_URL = os.getenv("WEBSITE_URL", "https://your-domain.example")
...
async def _cleanup_throttle_cache():
    """Периодически чистит _last_action_ts от старых записей, чтобы словарь не рос бесконечно."""
    while True:
...
# Инициализация локальной SQLite-базы для пользователей, инвайтов и рефералов.
def init_db():
