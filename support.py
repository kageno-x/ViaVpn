"""Бот поддержки для работы с тикетами пользователей.

Этот модуль отвечает за:
- создание и управление тикетами поддержки;
- переписку между пользователем и администратором;
- привязку тикетов к информации о пользователе из основного бота.
"""
...
# Загружаем переменные окружения из .env и qq.env, если они доступны.
# Это удобно для локального запуска и для Docker-контейнеров.
load_dotenv(dotenv_path=".env", override=False)
load_dotenv(dotenv_path="qq.env", override=False)
...
async def _cleanup_throttle_cache():
    """Периодически чистит _last_action_ts и неиспользуемые локи тикетов,
    чтобы словари не росли бесконечно."""
    while True:
...
# Инициализация локальной SQLite-базы для тикетов и сообщений поддержки.
def init_db():
    ...
    cursor.execute("""
        CREATE TABLE IF NOT EXISTS ticket_messages (
            ...
            sender_id INTEGER,
            text TEXT,
            forwarded_msg_id INTEGER,     -- id форварднутого юзер-сообщения в чате админа (для reply-роутинга)
            admin_chat_id INTEGER,        -- кому именно из админов было переслано это сообщение
            created_at TEXT,
...
def is_admin_reply(message: Message) -> bool:
    """Фильтр aiogram: True только если сообщение — Reply от админа (и не команда /close,
    которая обрабатывается отдельным хендлером выше). Если условие не выполняется,
    aiogram передаёт событие следующему подходящему хендлеру."""
...
def is_user_content_message(message: Message) -> bool:
    """Фильтр: обычное содержательное сообщение (текст/фото/документ), не команда."""
